          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {projects.map((project) => {
                ...
                const mediumCount = project.findingsSummary.medium ?? 0;
                const hasScore = project.complianceScore > 0;

                return (
                  <Link
                    key={project.id}
                    href={`/projects/${project.id}`}
                    className="group relative bg-slate-800/60 backdrop-blur-sm rounded-xl p-6 border border-slate-700/50 hover:border-emerald-500/50 shadow-xl hover:shadow-emerald-500/10 transition-all"
                  >
                    {/* Top: Donut + Info */}
                    <div className="flex items-start gap-5 mb-4">
                      ...
                      <div className="flex-1 min-w-0 pt-1">
                        <div className="flex items-center gap-2">
                          ...
                          {project.isSample && (
                            <span className="px-2 py-0.5 text-xs font-medium bg-slate-700 text-slate-400 rounded border border-slate-600 shrink-0">
                              Sample
                            </span>
                          )}
                        </div>
                    ...
                    {project.status === 'scanning' && (
                      <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                        ...
                        <span className="text-blue-400 text-xs">Scanning in progress...</span>
                      </div>
                    )}
                    ...
                    {(criticalCount > 0 || highCount > 0 || mediumCount > 0) && (
                      <div className="flex items-center gap-2 flex-wrap mb-3">
                        ...
                        {highCount > 0 && (
                          <span className="px-2 py-0.5 bg-orange-500/10 text-orange-400 text-xs font-semibold rounded border border-orange-500/20">
                            {highCount} High
                          </span>
                        )}
                        {mediumCount > 0 && (
                          <span className="px-2 py-0.5 bg-yellow-500/10 text-yellow-400 text-xs font-semibold rounded border border-yellow-500/20">
                            {mediumCount} Medium
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-slate-700/50">
                      {project.lastScanAt ? (
                        <span className="text-xs text-slate-400">
                          Last scan: {new Date(project.lastScanAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                        </span>
                      ) : (
                        <span className="text-xs text-slate-500">Not scanned</span>
                      )}
                      {project.repoUrl && (
                        <a
